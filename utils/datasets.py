...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=416, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
                 cache_images=False, single_cls=False, split='TRAIN', coco=True, task=''):
        path = str(Path(path))  # os-agnostic
        ...
        if self.COCO:
            ...
            data_folder = 'VOC_dataset/'
            with open(os.path.join(data_folder, self.task + self.split + '_images20.json'), 'r') as j:
        ...
        """
        #print(self.COCO)
        if self.COCO: # If using COCO dataset
        ...
        else: # If using Pascal VOC dataset
            data_folder = 'VOC_dataset/'
            objects_file = os.path.join(data_folder, self.task + self.split + '_objects20.json')
            try:
                with open(objects_file, 'r') as j:
                    self.objects = json.load(j)
            except:
                print("File %s not found" %objects_file)
            assert len(self.img_files) == len(self.objects), 'Image files = %d while objects = %d' % (len(self.img_files), len(self.objects))
            pbar = tqdm(self.objects, desc='Caching labels') 
            ...
            for object in self.objects:
                if object:
                    ...
                    boxes = np.asarray(object['boxes'], dtype=np.float32)
                    # print(len(boxes[0]))
                    assert len(object['boxes'][0]) == 4
                    l = np.zeros((len(labels), 5), dtype=np.float32)
                    l[:, 0] = labels
                    # print(l)
                    l[:, 1:] = boxes
                    # print(l)
    ...
    def __getitem__(self, index):
        ...
        if self.mosaic:
            ...
            x = self.labels[index]
            """
                    Label from Pascal VOC json files are in x_min, y_min, x_max, y_max format.
                    For training YOLO we need to change it to x_center, y_center, width, height format and then the value is normalized to 0. - 1.
                    !!!! WIP: Clean the code !!!
            """
            if not self.COCO: # If you are using Pascal VOC dataset
                VOC = x.copy()
                x[:, 1] = (VOC[:, 1] + ((VOC[:, 3] - VOC[:, 1]) /2)) / w0
                x[:, 2] = (VOC[:, 2] + ((VOC[:, 4] - VOC[:, 2]) /2)) / h0
                x[:, 3] = (VOC[:, 3] - VOC[:, 1]) / w0
                x[:, 4] = (VOC[:, 4] - VOC[:, 2]) / h0
...
class load_auxiliary_imgs(Dataset):
    def __init__(self, directory, batch_size=16):
        bi = np.floor(np.arange(n) / batch_size).astype(np.int)  # batch index
        nb = bi[-1] + 1  # number of batches
        self.n = n
        self.batch = bi  # batch index of image
